<x-app-layout>
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        {{-- SEUL ADMIN RH PEUT AJOUTER (selon vos nouvelles règles) --}}
        @if(Auth::user()->role === 'admin_rh')
            <a href="{{ route('directions.create') }}" class="inline-flex items-center px-6 py-3 bg-blue-600 hover:bg-indigo-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-blue-200 transition-all active:scale-95">
                + Nouvelle Direction
            </a>
        @endif
    </div>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($directions as $direction)
                        <tr class="hover:bg-blue-50/30 transition-colors group">
                            <td class="px-6 py-5">
                                <span class="font-mono text-[11px] bg-slate-100 px-3 py-1 rounded-lg text-slate-600 font-bold">
                                    {{ $direction->code }}
                                </span>
                            </td>
                            <td class="px-6 py-5">
                                <div class="font-bold text-slate-800 text-sm">{{ $direction->nom }}</div>
                            ...
                            <td class="px-6 py-5 text-center">
                                ...
                                <span class="text-[9px] ml-1 text-slate-400">{{ $direction->est_synchronise ? 'OK' : 'En attente' }}</span>
                            </td>
                            <td class="px-6 py-5 text-right">
                                <div class="flex justify-end gap-2">
                                    
                                    {{-- LOGIQUE DE MODIFICATION CORRIGÉE --}}
                                    @php
                                        $user = Auth::user();
                                        $canEdit = false;
                                        ...
                                        // Un admin DG ou Direction ne modifie QUE sa propre direction (via direction_id)
                                    ...
                                    @if($canEdit)
                                        <a href="{{ route('directions.edit', $direction->id) }}" title="Modifier" class="p-2 text-indigo-600 hover:bg-indigo-50 rounded-xl transition">
                                    ...
                                    {{-- LIEN VOIR (Utilise l'ID UUID) --}}
                                    <a href="{{ route('directions.show', $direction->id) }}" title="Détails" class="p-2 text-slate-400 hover:bg-slate-100 rounded-xl transition">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
